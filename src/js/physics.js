...
export class Physics {
    ...
    makeGround(){
        const floorMesh = new THREE.Mesh( new THREE.PlaneGeometry(100, 100, 10, 10), new THREE.MeshStandardMaterial())
    ...
    addCube(l, b, h, x, y, z){
        const cubeMesh = new THREE.Mesh( new THREE.BoxGeometry(l, b, h), new THREE.MeshStandardMaterial());
        cubeMesh.castShadow = true;
        this.threejs.addToScene(cubeMesh);
        const cubeBody = this.world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(x, y, z).setCanSleep(false))
        const cubeShape = RAPIER.ColliderDesc.cuboid(l/2, b/2, h/2).setMass(1).setRestitution(0.5);
        this.world.createCollider(cubeShape, cubeBody);
        this.dynamicBodies.push([cubeMesh, cubeBody])
    }

    addCylinder(r, h, x, y, z){
        const cylinderMesh = new THREE.Mesh(new THREE.CylinderGeometry(r, r, h ), new THREE.MeshNormalMaterial());
        cylinderMesh.castShadow = true;
        this.threejs.addToScene(cylinderMesh);
        const cylinderBody = this.world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(x, y, z).setCanSleep(false))
        const cylinderShape = RAPIER.ColliderDesc.cylinder(h/2, r).setMass(1).setRestitution(0.2);
